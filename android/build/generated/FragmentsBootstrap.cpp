...
#define TAG "com.yallaya.fragments"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Fragments_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Fragments.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::FragmentsBindings::lookupGeneratedInit(
...
static void Fragments_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Fragments_getBinding)->GetFunction());
}

static void Fragments_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::FragmentsBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry FragmentsBinding = {
...
Java_com_yallaya_fragments_FragmentsBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.yallaya.fragments", &FragmentsBinding);
	titanium::KrollBindings::addExternalLookup(&(::FragmentsBindings::lookupGeneratedInit));
}
